...
def _formato_libro(libro, indent=""):
    ...
    return f"{indent}ID: {libro['id']} - '{libro['titulo']}' ({libro['autor']}, {libro['año']}) [{estado}]"
...
def agregar_libro():
    ...
    while True:
        año = input("Ingrese el año de publicación (numérico y mayor a 1900): ")
        if año.isdigit() and int(año) > 1900:
            año = int(año)
            break
        else:
            print("Año inválido. Por favor, ingrese un año numérico mayor a 1900.")
    ...
    print(f"Libro '{titulo}' agregado exitosamente con ID {contador_id}.")
...
def buscar_libro():
    busqueda = input("\nIngrese el título o autor del libro a buscar: ").lower()
    encontrados = [
    ...
    if encontrados:
        print(f"Libros encontrados ({len(encontrados)}):")
    ...
    for libro in biblioteca:
        if libro["id"] == id:
            if not libro["disponible"]:
                ...
                print("El libro ya está disponible (aún no se ha prestado).")
            return
    print("No se encontró el libro con el ID proporcionado.")
...
def exportar_a_txt():
    ...
    print(" Libros exportados a biblioteca.txt exitosamente.")
...
def menu_principal():
    while True:
        ...
        if opcion == "1":
            ...
            prestar_libro(int(input("Ingrese el ID del libro a prestar: ")))
        elif opcion == "5":
            devolver_libro(int(input("Ingrese el ID del libro a devolver: ")))
        elif opcion == "6":
            eliminar_libro(int(input("Ingrese el ID del libro a eliminar: ")))
        elif opcion == "7":
            libros_por_autor(input("Ingrese el nombre del autor: "))
        elif opcion == "8":
            estadisticas()
        elif opcion == "9":
            exportar_a_txt()
